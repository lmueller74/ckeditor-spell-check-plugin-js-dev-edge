...
(function() {
	...
	CKEDITOR.plugins.add('nanospell', {
		...
		init: function(editor) {
		    ...
		    var spell_delay = 1000;
		    var spell_fast_after_spacebar = false;
		    var settings;
		    var settings_path;
		    var state = false;
		    var lang = "en";
            ...
            var afterInit = function (editor) {
			    ...
			    if (htmlFilter) htmlFilter.addRules(wiggles_filter);
		    ...
		    editor.addCommand('nanospell', {
			    exec: function(editor) {
			        setUpContextMenu(editor, settings_path);
			        if (!state) start();
			        else stop();
			    },
			    editorFocus: true
            ...
            function setUpContextMenu(editor, path) {
			    ...
			    log(editor.name + " = .contextMenu");
                log(editor.contextMenu);
		    ...
		    editor.stopNanospell = function() { stop(); }
		    editor.startNanospell = function () { log("startNanospell"); start(); }
	        ...
	        function parseRpc(data, words) {
		        try {
			        ...
			        if(window.location.href.indexOf('nanospell/')<0) {
			            ...
			            if (confirm(msg)) window.location = settings_path + "../getstarted.html";
			        }
	            }
	        ...
	        function render() {
	            log("render");
		        if(window.getSelection && editor.getWin().getSelection().toString()) return;
	        ...
	        function normalizeTextNodes(elem) {
		        ...
		        var child = elem.firstChild, nextChild;
		        while (child) {
			        if (child.nodeType == 3) {
	        ...
	        function getCaretIE() {
		        if (editor.getWin().getSelection) return null;
	        ...
	        function selectionCollapsed() {
		        if (!editor.getSelection()) return true;
	        ...
	        function triggerSpelling(immediate) {
		        ...
		        if (selectionCollapsed()) {
			        ...
			        spell_ticker = setTimeout(checkNow, immediate ? 1000 : spell_delay);
		        }
	        }
		    ...
		    if(!editor.nanospell) {
			    editor.nanospell = {
				    ...
				    setLanguage: function(langcode) { lang = langcode; checkNow(); }
			    };
		    }
        }
